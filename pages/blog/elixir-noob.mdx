...
export default ({children}) => (
  <Layout title="Coming to Elixir from TypeScript">{children}</Layout>
);
...
JavaScript is asynchronous (non-blocking) by default. This can be a bit tricky for new programmers, but it can be quite powerful once you get the hang of it (e.g. `Promise.all` is a nice way to execute a bunch of async processes concurrently).

Elixir is synchronous (blocking) by default — which makes things much easier, in my opinion — but Elixir also happens to make it incredibly easy to handle processes asynchronously if you would like to.
