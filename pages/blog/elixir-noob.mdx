...
JavaScript is asynchronous (non-blocking) by default. This can be a bit tricky for new programmers, but it can be quite powerful once you get the hang of it (e.g. `Promise.all` to execute a bunch of async processes concurrently).

Elixir is synchronous (blocking) by default -- which makes things much easier, in my opinion -- but it also happens to make it incredibly easy to handle processes asynchronously if you would like to.
...
So without further ado, here are the things I'm still getting used to with Elixir. 😬
...
Anyway, on one hand, I feel like this convention of the language is good because it forces programmers to be more cognizant of error handling. But it definitely takes some getting used to.

### Implicit returns (and no `return` keyword)

While pattern matching is great and all, the fact that Elixir does not have the ability to break out of a function early takes some getting used to.
...
For someone approaching Elixir as a newbie like myself, this can take some getting used to, because it forces you to take a step back and rethink how you would normalize design your functions.
...
I honestly have no idea whether Elixir is growing, stagnating, or declining in popularity, but so far things seem much less painful than I expected.
